...
DNSResolver::DNSResolver(EventLoop* evloop, const std::string& h, Duration timeout, const Functor& f)
    ...
    EVPP_DLOG_TRACE << "tid=" << std::this_thread::get_id() << " this=" << this;
}
...
void DNSResolver::SyncDNSResolve() {
    ...
    if (err != 0) {
        ...
        for (struct addrinfo* rp = answer; rp != nullptr; rp = rp->ai_next) {
            ...
            EVPP_DLOG_TRACE << "host=" << host_ << " resolved a ip=" << inet_ntoa(a->sin_addr);
        }
    }
...
void DNSResolver::AsyncDNSResolve() {
    ...
    EVPP_DLOG_TRACE << "call shared_from_this";
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr) {
    if (errcode != 0) {
        if (errcode != EVUTIL_EAI_CANCEL) {
            ...
            EVPP_DLOG_WARN << "DNS resolve cancel, may be timeout";
        }

        EVPP_DLOG_WARN << "delete DNS base. errcode=" << errcode << " " << strerror(errcode);
    ...
    if (addr == nullptr) {
        ...
        EVPP_DLOG_TRACE << "delete dns base";
        evdns_base_free(dnsbase_, 0);
        dnsbase_ = nullptr;
        ClearTimer();
        OnResolved();
    ...
    if (addr->ai_canonname) {
        EVPP_DLOG_TRACE << "resolve canon name: " << addr->ai_canonname;
    }
