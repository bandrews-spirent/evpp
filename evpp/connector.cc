...
Connector::Connector(EventLoop* l, TCPClient* client)
    ...
    EVPP_DLOG_TRACE << "raddr=" << remote_addr_;
...
Connector::~Connector() {
    ...
    if (status_ == kDNSResolving) {
        ...
        EVPP_DLOG_TRACE << "close(" << chan_->fd() << ")";
        assert(own_fd_);
        assert(chan_->fd() == fd_);
...
void Connector::Start() {
    EVPP_DLOG_TRACE << "Try to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    ...
    EVPP_DLOG_TRACE << "The remote address " << remote_addr_ << " is a host, try to resolve its IP address.";
...
void Connector::Cancel() {
    EVPP_DLOG_TRACE << "Cancel to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    if (dns_resolver_) {
...
void Connector::Connect() {
    EVPP_DLOG_TRACE << remote_addr_ << " status=" << StatusToString();
    ...
    EVPP_DLOG_TRACE << "new FdChannel p=" << chan_.get() << " fd=" << chan_->fd();
...
void Connector::HandleError() {
    ...
    if (owner_tcp_client_->auto_reconnect()) {
        ...
        if (fd_ > 0) {
            EVPP_DLOG_TRACE << "Connector::HandleError close(" << fd_ << ")";
            assert(own_fd_);
            EVUTIL_CLOSESOCKET(fd_);
            fd_ = INVALID_SOCKET;
        }

        EVPP_DLOG_TRACE << "loop=" << loop_ << " auto reconnect in " << owner_tcp_client_->reconnect_interval().Seconds() << "s thread=" << std::this_thread::get_id();
...
void Connector::OnDNSResolved(const std::vector <struct in_addr>& addrs) {
    EVPP_DLOG_TRACE << "addrs.size=" << addrs.size() << " this=" << this;
    if (addrs.empty()) {
