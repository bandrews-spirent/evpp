...
void EventLoop::Run() {
    ...
    watcher_.reset();
    EVPP_DLOG_TRACE << "EventLoop stopped, tid=" << std::this_thread::get_id();
...
void EventLoop::Stop() {
    ...
    EVPP_DLOG_TRACE << "EventLoop::Stop";
    QueueInLoop(std::bind(&EventLoop::StopInLoop, this));
}

void EventLoop::StopInLoop() {
    EVPP_DLOG_TRACE << "EventLoop is stopping now, tid=" << std::this_thread::get_id();
    assert(status_.load() == kStopping);

    auto f = [this]() {
        for (int i = 0;;i++) {
            EVPP_DLOG_TRACE << "calling DoPendingFunctors index=" << i;
            DoPendingFunctors();
    ...
    EVPP_DLOG_TRACE << "before event_base_loopexit, we invoke DoPendingFunctors";

    f();

    EVPP_DLOG_TRACE << "start event_base_loopexit";
    event_base_loopexit(evbase_, nullptr);
    EVPP_DLOG_TRACE << "after event_base_loopexit, we invoke DoPendingFunctors";

    f();

    EVPP_DLOG_TRACE << "end of StopInLoop";
}
...
void EventLoop::QueueInLoop(const Functor& cb) {
    EVPP_DLOG_TRACE << "pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    {
#ifdef H_HAVE_BOOST
    ...
    EVPP_DLOG_TRACE << "queued a new Functor. pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    if (!notified_.load()) {
        EVPP_DLOG_TRACE << "call watcher_->Nofity() notified_.store(true)";
        ...
        if (watcher_) {
            ...
            EVPP_DLOG_TRACE << "status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
         EVPP_DLOG_TRACE << "No need to call watcher_->Nofity()";
    }
}
...
void EventLoop::QueueInLoop(Functor&& cb) {
    ...
    if (!notified_.load()) {
        ...
        if (watcher_) {
            ...
            EVPP_DLOG_TRACE << "watcher_ is empty, maybe we call EventLoop::QueueInLoop on a stopped EventLoop. status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
        EVPP_DLOG_TRACE << "No need to call watcher_->Nofity()";
