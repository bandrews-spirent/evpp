...
TCPClient::TCPClient(EventLoop* l, const std::string& raddr, const std::string& n)
    ...
    EVPP_DLOG_TRACE << "remote addr=" << raddr;
}
...
void TCPClient::DisconnectInLoop() {
    ...
    if (conn_) {
        EVPP_DLOG_TRACE << "Close the TCPConn " << conn_.get() << " status=" << conn_->StatusToString();
    ...
    if (connector_->IsConnected() || connector_->IsDisconnected()) {
        EVPP_DLOG_TRACE << "Nothing to do with connector_, Connector::status=" << connector_->status();
    } else {
...
void TCPClient::Reconnect() {
    EVPP_DLOG_TRACE << "Try to reconnect to " << remote_addr_ << " in " << reconnect_interval_.Seconds() << "s again";
    Connect();
}
...
void TCPClient::OnConnection(evpp_socket_t sockfd, const std::string& laddr) {
    if (sockfd < 0) {
        EVPP_DLOG_TRACE << "Failed to connect to " << remote_addr_ << ". errno=" << errno << " " << strerror(errno);
    ...
    EVPP_DLOG_TRACE << "Successfully connected to " << remote_addr_;
    assert(loop_->IsInLoopThread());
