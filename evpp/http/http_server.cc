...
bool Server::Start() {
    ...
    for (auto& lt : listen_threads_) {
        ...
        auto http_close_fn = [hservice, this]() {
            hservice->Stop();
            EVPP_DLOG_TRACE << "http service at 0.0.0.0:" << hservice->port() << " has stopped.";
    ...
    EVPP_DLOG_TRACE << "http server is running" ;
...
void Server::Stop() {
    assert(IsRunning());
    EVPP_DLOG_TRACE << "http server is stopping";
    ...
    EVPP_DLOG_TRACE << "http server stopped";
}

void Server::Pause() {
    EVPP_DLOG_TRACE << "http server pause";
    for (auto& lt : listen_threads_) {
...
void Server::Continue() {
    EVPP_DLOG_TRACE << "http server continue";
    for (auto& lt : listen_threads_) {
        EventLoop* loop = lt.thread->loop();
        std::shared_ptr<Service>& hs = lt.hservice;
        auto f = [hs]() {
...
void Server::Dispatch(EventLoop* listening_loop,
    ...
    EVPP_DLOG_TRACE << "dispatch request " << ctx->req() << " url=" << ctx->original_uri() << " in main thread. status=" << StatusToString();
    if (!IsRunning()) {
    ...
    auto f = [loop, ctx, response_callback, user_callback, this]() {
        EVPP_DLOG_TRACE << "process request " << ctx->req()
