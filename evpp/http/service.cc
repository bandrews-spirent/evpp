...
#define H_ARRAYSIZE(a) \
        ...
        bool Service::initSSL(bool force_enable) {
            EVPP_DLOG_TRACE << "https service init ssl";
            if(force_enable) {
        ...
        void Service::Stop() {
            EVPP_DLOG_TRACE << "http service is stopping";
            assert(listen_loop_->IsInLoopThread());
            ...
            EVPP_DLOG_TRACE << "http service stopped";
        }


        void Service::Pause() {
            assert(listen_loop_->IsInLoopThread());
            EVPP_DLOG_TRACE << "http service pause";
#if LIBEVENT_VERSION_NUMBER >= 0x02001500
            if (evhttp_bound_socket_) {
        ...
        void Service::Continue() {
            assert(listen_loop_->IsInLoopThread());
            EVPP_DLOG_TRACE << "http service continue";
#if LIBEVENT_VERSION_NUMBER >= 0x02001500
            if (evhttp_bound_socket_) {
        ...
        void Service::HandleRequest(struct evhttp_request* req) {
            ...
            assert(listen_loop_->IsInLoopThread());
            EVPP_DLOG_TRACE << "handle request " << req << " url=" << req->uri;
        ...
        void Service::DefaultHandleRequest(const ContextPtr& ctx) {
            EVPP_DLOG_TRACE << "url=" << ctx->original_uri();
        ...
        void Service::SendReply(const ContextPtr& ctx, const std::string& response_data) {
            ...
            EVPP_DLOG_TRACE << "send reply in working thread";
            ...
            auto f = [this, response]() {
                // In the main HTTP listening thread
                assert(listen_loop_->IsInLoopThread());
                EVPP_DLOG_TRACE << "send reply in listening thread. evhttp_=" << evhttp_;
            ...
            if (listen_loop_->IsRunning()) {
                EVPP_DLOG_TRACE << "dispatch this SendReply to listening thread";
