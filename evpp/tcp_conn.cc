...
TCPConn::TCPConn(EventLoop* l,
    ...
    EVPP_DLOG_TRACE << "TCPConn::[" << name_ << "] channel=" << chan_.get() << " fd=" << sockfd << " addr=" << AddrToString();
}

TCPConn::~TCPConn() {
    EVPP_DLOG_TRACE << "name=" << name()
        << " channel=" << chan_.get()
...
void TCPConn::Close() {
    EVPP_DLOG_TRACE << "fd=" << fd_ << " status=" << StatusToString() << " addr=" << AddrToString();
...
void TCPConn::HandleRead() {
    ...
    if (n > 0) {
        ...
        if (type() == kOutgoing) {
            ...
            EVPP_DLOG_TRACE << "fd=" << fd_ << ". We read 0 bytes and close the socket.";
            status_ = kDisconnecting;
            HandleClose();
        } else {
            ...
            if (close_delay_.IsZero()) {
                EVPP_DLOG_TRACE << "channel (fd=" << chan_->fd() << ") DisableReadEvent. delay time " << close_delay_.Seconds() << "s. We close this connection immediately";
                DelayClose();
            } else {
                ...
                EVPP_DLOG_TRACE << "channel (fd=" << chan_->fd() << ") DisableReadEvent. And set a timer to delay close this TCPConn, delay time " << close_delay_.Seconds() << "s";
        ...
        if (EVUTIL_ERR_RW_RETRIABLE(serrno)) {
            EVPP_DLOG_TRACE << "errno=" << serrno << " " << strerror(serrno);
        } else {
            EVPP_DLOG_TRACE << "errno=" << serrno << " " << strerror(serrno) << " We are closing this connection now.";
            HandleError();
        }
    }
}
...
void TCPConn::DelayClose() {
    assert(loop_->IsInLoopThread());
    EVPP_DLOG_TRACE << "addr=" << AddrToString() << " fd=" << fd_ << " status_=" << StatusToString();
    status_ = kDisconnecting;
...
void TCPConn::HandleClose() {
    ...
    if (delay_close_timer_) {
        EVPP_DLOG_TRACE << "loop=" << loop_ << " Cancel the delay closing timer.";
    ...
    EVPP_DLOG_TRACE << "addr=" << AddrToString() << " fd=" << fd_ << " status_=" << StatusToString() << " use_count=" << conn.use_count();
    status_ = kDisconnected;
}

void TCPConn::HandleError() {
    EVPP_DLOG_TRACE << "fd=" << fd_ << " status=" << StatusToString();
    status_ = kDisconnecting;
    HandleClose();
}
