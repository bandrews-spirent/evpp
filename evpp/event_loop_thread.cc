...
EventLoopThread::EventLoopThread()
    ...
    EVPP_DLOG_TRACE << "loop=" << event_loop_;
}
...
void EventLoopThread::Run(const Functor& pre, const Functor& post) {
    ...
    EVPP_DLOG_TRACE << "loop=" << event_loop_ << " execute pre functor.";
    auto fn = [this, pre]() {
        status_ = kRunning;
        if (pre) {
            auto rc = pre();
    ...
    EVPP_DLOG_TRACE << "loop=" << event_loop_ << " execute post functor.";
    if (post) {
        post();
    }

    assert(event_loop_->IsStopped());
    EVPP_DLOG_TRACE << "loop=" << event_loop_ << " EventLoopThread stopped";
    status_ = kStopped;
}

void EventLoopThread::Stop(bool wait_thread_exit) {
    EVPP_DLOG_TRACE << "loop=" << event_loop_ << " wait_thread_exit=" << wait_thread_exit;
    ...
    if (wait_thread_exit) {
        ...
        EVPP_DLOG_TRACE << "loop=" << event_loop_ << " thread stopped.";
        Join();
        EVPP_DLOG_TRACE << "loop=" << event_loop_ << " thread totally stopped.";
    }
    EVPP_DLOG_TRACE << "loop=" << event_loop_;
}
...
void EventLoopThread::Join() {
    ...
    if (thread_ && thread_->joinable()) {
        EVPP_DLOG_TRACE << "loop=" << event_loop_ << " thread=" << thread_ << " joinable";
        try {
            thread_->join();
