...
static void HandleHTTPResponse(const std::shared_ptr<evpp::httpc::Response>& response, evpp::httpc::GetRequest* request) {
    EVPP_LOG_INFO << "http_code=" << response->http_code() << " [" << response->body().ToString() << "]";
    std::string header = response->FindHeader("Connection");
    EVPP_LOG_INFO << "HTTP HEADER Connection=" << header;
    responsed = true;
...
int main(int argc, char* argv[]) {
    ...
    t.Stop(true);
    EVPP_LOG_INFO << "EventLoopThread stopped.";
    return 0;
}
