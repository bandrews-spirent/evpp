...
public:
    ...
    void OnDisconnect(const evpp::TCPConnPtr& conn) {
        if (--connected_count_ == 0) {
            ...
            EVPP_LOG_WARN << "name=" << name_ << " " << totalBytesRead << " total bytes read";
            EVPP_LOG_WARN << "name=" << name_ << " " << totalMessagesRead << " total messages read";
            EVPP_LOG_WARN << "name=" << name_ << " " << static_cast<double>(totalBytesRead) / static_cast<double>(totalMessagesRead) << " average message size";
            EVPP_LOG_WARN << "name=" << name_ << " " << static_cast<double>(totalBytesRead) / (timeout_ * 1024 * 1024) << " MiB/s throughput";
