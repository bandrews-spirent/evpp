...
static void RequestHandler909(evpp::EventLoop* loop, const evpp::http::ContextPtr& ctx, const evpp::http::HTTPSendResponseCallback& cb) {
    EVPP_LOG_INFO << "RequestHandler909";
    std::stringstream oss;
    oss << "func=" << __FUNCTION__ << " OK"
        << " ip=" << ctx->remote_ip() << "\n"
        << " uri=" << ctx->uri() << "\n"
...
int main() {
    ...
    if (pid != 0) {
        // In parent process 
        EVPP_LOG_INFO << "In parent process. Starting";
        ph.Start();
        EVPP_LOG_INFO << "In parent process. Stopping";
        ph.Stop();
        EVPP_LOG_INFO << "In parent process. Stopped";
        return 0;
    }
    
    EVPP_LOG_INFO << "In child process. Doing AfterFork";
    ph.AfterFork();
    ph.Start();
    H_TEST_ASSERT(r);
    Test909();
