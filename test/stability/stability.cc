...
static void RequestHandler(evpp::EventLoop* loop, const evpp::http::ContextPtr& ctx, const evpp::http::HTTPSendResponseCallback& cb) {
    EVPP_LOG_INFO << "DefaultRequestHandler loop=" << loop << " ctx.url=" << ctx->original_uri() << " tid=" << std::this_thread::get_id();
...
namespace {
    ...
    void testDefaultHandler1(evpp::EventLoop* loop, std::atomic<int>* finished) {
        ...
        auto f = [r, finished](const std::shared_ptr<evpp::httpc::Response>& response) {
            EVPP_LOG_INFO << "request=" << r << " response=" << response.get() << " tid=" << std::this_thread::get_id();
    ...
    static void TestAll() {
        ...
        t.Stop(true);
        EVPP_LOG_INFO << "TestAll end";
    }
}

void TestHTTPServer() {
    for (int i = 0; i < 40; ++i) {
        EVPP_LOG_INFO << "Running TestHTTPServer i=" << i;
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0;;i++) {
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServer1 " << i;
        TestTCPServer1();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestHTTPServer " << i;
        TestHTTPServer();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientReconnect " << i;
        TestTCPClientReconnect();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientConnectFailed " << i;
        TestTCPClientConnectFailed();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientDisconnectImmediately " << i;
        TestTCPClientDisconnectImmediately();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientDisconnectAndDestruct " << i;
        TestTCPClientDisconnectAndDestruct();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientConnectLocalhost " << i;
        TestTCPClientConnectLocalhost();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServerSilenceShutdown1 " << i;
        TestTCPServerSilenceShutdown1();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServerSilenceShutdown2 " << i;
        TestTCPServerSilenceShutdown2();
        EVPP_LOG_WARN << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestDNSResolver " << i;
        TestDNSResolver();
    }
    return 0;
}
