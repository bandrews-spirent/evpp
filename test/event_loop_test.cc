...
static void PeriodicFunc() {
    periodic_run_count++;
    EVPP_LOG_INFO << "PeriodicFunc is called , periodic_run_count=" << periodic_run_count;
}
}
...
void Run() {
    EVPP_LOG_INFO << "Running count=" << count << " ...";
    if (count++ == 5) {
        invoke_timer->Cancel();
        loop->Stop();
    }
...
TEST_UNIT(TestEventLoop5) {
    evpp::EventLoop loop;
    auto fn = [&loop]() {
        EVPP_LOG_INFO << "Entering fn";
        auto close = [&loop]() {
            EVPP_LOG_INFO << "Entering close";
            loop.Stop();
        };
